...
def gittool_makejob(config, jobname, rep, comment, testrun=False,invocation=None,gitignore=True):
    # max_procs=threads
    workdir = rep.workdir
...
def addcommand_makejob(parser=None, args=None):
    if parser is not None:
        ...
        makejob.add_argument('--no-gitignore', action='store_true')
    ...
    if args is not None:
        ...
        gittool_makejob(config=config,jobname=args.jobname,rep=rep,comment=args.comment,testrun=args.testrun,invocation=sys.argv,gitignore=not args.no_gitignore)
