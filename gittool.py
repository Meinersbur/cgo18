    ...
    if isinstance(onlychildren, dict) or (unlisted is not REV_KEEP): # don't recurse if it's not changing anything
      for subproject,subsubdir,subismodule,subcheckout in project.subprojects:
        if onlychildren is None:
            ...
            continue
        subrep = checkout_sub(subproject, projectdir, subsubdir, subismodule, rep, onlychildren=subonlychildren,checkout=subcheckout,ignore_fetch_errors=ignore_fetch_errors,enable_svn=enable_svn)
...
def gittool_checkout(projectstr):
    ...
    checkout_sub(project, srcdir, [project.name], ismodule= False)
        ...
        if writetest :
          ...
          if ll_filepath is not None:
            ...
            print("Reduced testcase written to",testfilepath)
                ...
                targetfilepath = testfileprefix + '.c'
...
class RuntestConfig:
    ...
    def add_runtest_arguments(parser):
         ...
         parser.add_argument('--suite-build-flags','--suite-flags', '--cppflags', action='append')
...
def runtest(config,print_logs=False,enable_svn=True):
    ...
    if suite_testonly is not None:
        ...
        known_configurable = { 'Performance', 'Performance/Polybench-32', 'Performance/Polybench-421', 'Correctness', 'Correctness/Polybench-32', 'Correctness/Polybench-421',
                               'External', 'External/SPEC',
                               'SingleSource', 'SingleSource/Applications', 'SingleSource/Benchmarks', 'SingleSource/UnitTests',
                               'MultiSource', 'MultiSource/Applications', 'MultiSource/Benchmarks', 'MultiSource/Examples', 'MultiSource/UnitTests', 'MultiSource/Applications/JM','MultiSource/Applications/JM/lencod'}
        build_testonly = set()
        exec_testonly = set()
        ...
        for dir in suite_testonly:
            ...
            build_testonly.add(subdir)
            exec_testonly.add(dir)

    if build_testonly:
        print("configure tests  :", ' '.join(build_testonly))
    if exec_testonly:
    ...
    def get_lit_args(i=None):
        ...
        litopts += ['-s'] # Reduce amount of output
    ...
    def suite_configure():
        ...
        if build_testonly is not None:
               cmakedefs += ['TEST_SUITE_SUBDIRS=' +';'.join(build_testonly)]
...
def gittool_execslave(workdir, source, target, persistentdir, slavename, waitsecs=datetime.timedelta(minutes=5),onlyone=False,use_perf=None, llvmcmake=[], lntargs=[],testsuitecmake=[], testsuiteflags=[], testsuitemllvm=[],exec_cpus=None,compile_cpus=None,configname=None,llvmflags=[],onlyjob=None,watch=True):
    ...
    # Update results, to not overwrite older results when pushing, maybe
    # because this is a pristine workdir
    ...
    while True:
        ...
        targetremote.fetch(prune=True)
        #targetremote.fetch(prune=True, refspec=[branch.get_basename() for
        #branch in srcbranches])
        ...
        for srcbranch in srcbranches:
            ...
            with TemporaryDirectory(prefix='slave-') as tmpdir, TemporaryDirectory(prefix='slavemsg-') as msgtmpdir:
                ...
                if dstbranch.exists():
                    rep.merge(dstbranch, noedit=True, strategy='ours', nocommit=True)
                ...
                #success,rtncode = invoke(exe, *args, cwd=rep.workdir,
                #addenv=addenv, stdout=stdoutfile, stderr=stderrfile,
                #resumeonerror=True)
                ...
                # Make a target commit
                #if dsthead is not None:
                #    rep.reset(dsthead, index=True, workdir=False)
                #    rep.invoke_git('checkout', '-B', dstname)
                #    curbranch = Branch(rep, dstname)
                #else:
                #    curbranch = Branch(rep, dstname)
                #    curbranch.remove(resumeonerror=True)
                #    rep.invoke_git('checkout', '--orphan', dstname)
        ...
        if not foundany:
            if not watch:
                print("No addition job found. exiting...")
                return
...
def addcommand_execslave(parser=None, args=None):
    ...
    if args is not None:
        ...
        gittool_execslave(workdir=workdir, source=args.source, target=args.target, persistentdir=args.persistentdir, slavename=args.slavename,waitsecs=datetime.timedelta(seconds=args.waitsecs),onlyone=args.onlyone,onlyjob=args.onlyjob,watch=not args.no_watch)
